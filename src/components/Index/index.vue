  ...
  <div id="index" style="height:100%;">
    <view-box body-padding-bottom="48px" body-padding-top="0px">
      <swiper :aspect-ratio="500/900" auto>
        <swiper-item class="swiper-demo-img" v-for="(item, index) in headerinfo.banner" :key="index">
          <a :href="item.url">
            <img :src="item.img">
          </a>
        </swiper-item>
      ...
      <div v-transfer-dom>
        <popup v-model="showPopup" class="checker-popup">
          <div style="padding:10px 10px 40px 10px;">
            ...
            <checker v-model="type" default-item-class="demo4-item" selected-item-class="demo4-item-selected" disabled-item-class="demo4-item-disabled">
              <checker-item :value="item" v-for="(item, key) in types" :key="key" @on-item-click="onItemClick"> {{item}} </checker-item>
            </checker>
          </div>
        </popup>
      </div>
      <sticky scroll-box="vux_view_box_body" ref="sticky" :check-sticky-support="false" :offset = "height">
        <group label-align="justify" gutter="0px">
      ...
      <group slot="default" gutter="0px">
        <cell v-for="(item, index) in headerinfo.data" :key="index" @click.native="doctorInfo(item)" v-show="onTypeFilter(item.type, type)">
          ...
          <div class="content-l" slot="after-title">
            <p>
              <i class="iconfont icon-mine_fill"></i>{{item.name}}</p>
            <p>
              <i class="iconfont icon-mine_fill"></i>{{item.type}}</p>
            <p>
              <i class="iconfont icon-coordinates_fill"></i>{{item.address}}</p>
            <p>
              <i class="iconfont icon-mobilephone_fill"></i>{{item.phone}}</p>
            <p>
